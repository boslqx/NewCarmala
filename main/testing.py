...
import folium
import webview  # To display the map
import os


# Function to generate and open Google Map for selecting pickup location
def open_map():
    # Create a folium map centered at a default location
    m = folium.Map(location=[3.1390, 101.6869], zoom_start=13)  # Default center (Kuala Lumpur)

    # Save the map to an HTML file
    map_file = "pickup_map.html"
    m.save(map_file)

    # Open the map in a WebView window
    webview.create_window("Select Pickup Location", map_file)
    webview.start()
...
# Create the "Select Location on Map" button next to the location entry
map_button = ttk.Button(home_tab, text="Select Location on Map", command=open_map)
canvas.create_window(450, 600, anchor="nw", window=map_button)  # Positioned beside the location entry

pickup_label = tk.Label(home_tab, text="Pickup date", font=("Helvetica", 12), bg="white")
canvas.create_window(600, 600, anchor="nw", window=pickup_label)

# Replace the text entry with a calendar date picker (DateEntry)
pickup_date_entry = DateEntry(home_tab, font=("Helvetica", 12), width=18, background='darkblue', foreground='white',
                              borderwidth=2)
canvas.create_window(700, 600, anchor="nw", window=pickup_date_entry)

return_label = tk.Label(home_tab, text="Return date", font=("Helvetica", 12), bg="white")
canvas.create_window(880, 600, anchor="nw", window=return_label)

# Replace the text entry with a calendar date picker (DateEntry)
return_date_entry = DateEntry(home_tab, font=("Helvetica", 12), width=18, background='darkblue', foreground='white',
                              borderwidth=2)
canvas.create_window(980, 600, anchor="nw", window=return_date_entry)
