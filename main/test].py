...
# Function to display car details in a new window
def show_car_details(booking_id):
    ...
    if car_details:
        ...
        # Adjust the unpacking based on the actual structure of CarList
        if len(car_details) >= 6:  # Assuming CarList has at least 6 columns
            car_id, car_name, car_type, car_brand, car_price, car_image, *extra = car_details
        else:
            messagebox.showerror("Error", "Invalid data in CarList table.")
            return

        # Display car details in a new window
        car_window = tk.Toplevel(booking_window)
        car_window.title("Car Details")
        car_window.geometry("500x500")

        tk.Label(car_window, text=f"Car Name: {car_name}", font=("Poppins", 12)).pack(pady=10)
        tk.Label(car_window, text=f"Car Type: {car_type}", font=("Poppins", 12)).pack(pady=10)
        tk.Label(car_window, text=f"Car Brand: {car_brand}", font=("Poppins", 12)).pack(pady=10)
        tk.Label(car_window, text=f"Price: ${car_price}", font=("Poppins", 12)).pack(pady=10)
        ...
        if car_image and os.path.exists(car_image):
            ...
            image_label = tk.Label(car_window, image=photo)
            image_label.image = photo  # Keep a reference to avoid garbage collection
            image_label.pack(pady=20)
        else:
            tk.Label(car_window, text="Image not available", font=("Poppins", 12)).pack(pady=10)
    else:
